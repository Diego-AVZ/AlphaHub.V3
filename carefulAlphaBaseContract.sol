...
contract A {
    ...
    /////////////
    address public b = 0x1c91347f2A44538ce62453BEBd9Aa907C662b4bD;
...
contract B {
    ...
    function setActualPeriod2() public returns(uint){ //onlyOwner
...
/* 
    function addMsg(uint16 sigId, string memory newMsg) public {
        uint16 i =  findTraSignalIndex(sigId);
        traSignal storage traSigAlpha = alphaTradInfoFromAddress[msg.sender][i];
        require(traSigAlpha.alpha == msg.sender);
        traSigAlpha._msg.push(newMsg);
    }
*/
    ...
    function getActiveAlphaTra(uint32 i) public view returns(address){
...
contract C {
    ...
    mapping(uint => address[]) public oncActiveAlphaProvsThisPeriod;
    ...
    function addOncSignal(string memory asset, address tokenAddr, string memory _priceEntry, string memory _stopLoss, string memory _takeProfit, uint8 _direction, string memory _msg) public {
        ...
        oncActiveAlphaProvsThisPeriod[actualPeriod].push(msg.sender);
        uint perAccuracy =  accuracyPercentage(msg.sender);
    ...
    function getActiveAlphaOnc() public view returns(address[] memory){
        return(oncActiveAlphaProvsThisPeriod[actualPeriod]);
    }

    function getActiveAlphaOncLen() public view returns(uint){
        return(oncActiveAlphaProvsThisPeriod[actualPeriod].length);
    }
